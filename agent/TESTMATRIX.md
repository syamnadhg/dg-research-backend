...
suite with `pytest` (646 tests; ruff clean). The bridge is the single owner of
...
| Versions + "newer" notices (skill only; backend version shown, no backend nudge) | `GET /version`, `GET /status` | `test_sr_client`, `test_selfupdate` |
