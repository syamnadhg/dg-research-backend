...
class TestRestartCommand:
    ...
    def test_restart_needs_no_firestore_rules_change(self):
        """The device-commands allowlist is default-closed
        (`action in ["hard_reset", "check-update"] || isDeviceOwner()`), so `restart`
        is owner-only automatically. Pinned here because a future release that ADDS
        restart to that allowlist would silently hand a sharer the ability to cycle
        someone else's backend."""
        rules = Path(__file__).resolve().parents[2] / "dg-research" / "firestore.rules"
        if not rules.exists():
            pytest.skip("frontend repo not checked out beside the backend")
        text = rules.read_text(encoding="utf-8")
        assert 'request.resource.data.action in ["hard_reset", "check-update"]' in text
        assert '"restart"' not in text.split("ALLOWLIST")[1][:900], (
            "restart must stay owner-only (absent from the sharer allowlist)"
        )


class TestRestartCannotStrandTheMachine:
    """`restart` exits the worker. If nothing supervises the box, it never comes back —
    a button press would take the backend permanently offline. `update` has always
    guarded this ("a foreground --serve has nothing to bring it back"); the new
    `restart` command shipped without it until the pre-PR review."""

    def _branch(self) -> str:
        src = inspect.getsource(research._start_device_command_listener)
        blk = src[src.index('if action == "restart":'):]
        return blk[:blk.index("_schedule_server_exit") + 200]

    def test_refuses_when_unsupervised(self):
        blk = self._branch()
        assert "_detect_supervised()" in blk, (
            "an unsupervised restart strands the machine offline"
        )
        i_guard = blk.index("_detect_supervised()")
        i_exit = blk.index("_schedule_server_exit")
        assert i_guard < i_exit, "the guard must precede the exit"

    def test_tells_the_app_why_it_refused(self):
        blk = self._branch()
        # Both refusal paths (unsupervised, mid-run) must write a terminal status, or
        # the About row spins with no explanation.
        assert blk.count("_write_update_status") >= 2
        assert "not supervised" in blk and "run is in progress" in blk
