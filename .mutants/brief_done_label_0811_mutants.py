...
MUTANTS = [
    ...
    # ── the label-free completion path ──────────────────────────────────────
    ("S1", "under", "the label-free path is switched off entirely",
     [('                    if (not _sn_is_complete and _sn_verdict == "ambiguous"',
       '                    if (False and not _sn_is_complete and _sn_verdict == "ambiguous"')]),
    ...
    ("S4", "over", "one flat poll is enough — no window required",
     [("                            and (time.time() - stall_window_start) >= SAFETY_NET_CUA_SEC):",
       "                            and (time.time() - stall_window_start) >= 0):")]),
    ...
    ("S6", "over", "state out-votes a positively observed 'still generating'",
     [('                    if (not _sn_is_complete and _sn_verdict == "ambiguous"',
       '                    if (not _sn_is_complete and _sn_verdict in ("ambiguous", "generating")')]),
