...
('N7', RP, 'over',
    ...
    ('ST1', RP, 'under',
     '⛔⛔ A STOP ALREADY PRESSED STILL FETCHES: a skipped or salvaged partial waits on its images, the process exits 3 s after the Stop, and the document said to be saved is never written',
     [('        if _controls.is_stop():\n            raise _DocImgStopRequested()\n        pool = _doc_img_executor()',
       '        pool = _doc_img_executor()')]),
    ('ST2', RP, 'under',
     '⛔⛔ A STOP DURING THE WAIT IS NEVER SEEN: the document waits for its fetches past the process exit',
     [('            if _controls.is_stop():\n                raise _DocImgStopRequested()\n    finally:',
       '    finally:')]),
    ('ST3', RP, 'under',
    ...
    ('ST4', RP, 'under',
     '⛔⛔ THE STOP GUARD WIDENS TO A PAUSE before the call: a paused finalize writes captions, and a caption never becomes an image again',
     [('        if _controls.is_stop():\n            raise _DocImgStopRequested()\n        pool',
       '        if _controls.is_stop_or_pause():\n            raise _DocImgStopRequested()\n        pool')]),
    ('ST5', RP, 'under',
     '⛔⛔ THE STOP GUARD WIDENS TO A PAUSE during the wait: a pause mid-fetch captions the document for good',
     [('            if _controls.is_stop():\n                raise _DocImgStopRequested()\n    finally:',
       '            if _controls.is_stop_or_pause():\n                raise _DocImgStopRequested()\n    finally:')]),
    ('ST6', RP, 'under',
